#!/usr/bin/env python3
"""Exercise production title progress setup, rendering, and seek isolation."""
...
constexpr int LV_EVENT_VALUE_CHANGED = 2, LV_EVENT_RELEASED = 3, LV_ANIM_OFF = 0;
...
lv_obj_t *setup_slider_widget(lv_obj_t *parent, uint32_t color, bool) {
...
int main() {
  ...
  lv_obj_t title;
  auto *slider = setup_media_progress_background(&title, grey, background, "media_player.test", false, accent);
  ...
  assert(ctx->fill->color == accent && ctx->fill->width == 0);
  // Timer refreshes advance even without new Home Assistant position reports.
  now_ms = 6000;
  media_playback_apply_state_to_slider(&playback, ctx);
  ...
  // The usual slider update callback must remain installed in read-only mode.
  slider->value = 40;
  ...
  now_ms = 51000;
  media_playback_apply_state_to_slider(&playback, ctx);
  assert(slider->value == 10 && ctx->fill->color == grey);
  playback.playing = true;
  playback.position_updated_ms = now_ms;
  now_ms += 10000;
  media_playback_apply_state_to_slider(&playback, ctx);
  assert(slider->value == 20 && ctx->fill->color == accent);
  playback.duration = 0;
  media_playback_apply_state_to_slider(&playback, ctx);
  assert(slider->value == 0 && ctx->fill->width == 0);
  lv_obj_t seek_title;
  auto *seek_slider = setup_media_progress_background(&seek_title, grey, background, "media_player.test");
  assert(seek_slider->clickable);
  emit(seek_slider, LV_EVENT_RELEASED);
  assert(seek_count == 1);
  for (auto *parent : {&title, &seek_title}) {
    delete static_cast<SliderCtx *>(parent->children[1]->data);
    for (auto *child : parent->children) delete child;
  }
}
'''
